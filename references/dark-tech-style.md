...
暗黑科技不是“每页都炫”，而是“关键几页有冲击，其余页面有节奏”。推荐顺序：

1. Big Claim：一句强主张或未来感判断
2. System Map：系统能力、信号流、技术路线
3. Capability Proof：核心能力、性能指标、关键特性
4. Process / Pipeline：数据流、模型流、执行链路
5. Vision Turn：黑底强转场或未来路线
6. Closing Impact：下一阶段、生态、场景扩张

视觉冲击应集中在封面、章节页、系统总览页，不要让每一页都在尖叫。

## 表达子模式

同一套风格内允许两种子模式，但必须选一种为主：

- **Launch Neon**：偏发布会，允许更强霓虹感、KPI 冲击、舞台型大字
- **Infra Cyber**：偏技术方案，减少发光，增加拓扑、流程、系统分层

如果内容是技术路线或解决方案，优先 `Infra Cyber`；如果是产品发布或能力秀，优先 `Launch Neon`。

## 签名页

这套风格的 signature pages 应优先是：

- **System Map / Topology**：技术系统总览
- **Pipeline Slide**：数据流、推理流、协同链路
- **KPI Neon Wall**：少量但高冲击的关键指标
- **Dark Statement Divider**：用来切节奏，而不是装饰

## 禁用模式

- 禁止每页都使用发光、霓虹边、强高亮。每页只允许 1 个主发光焦点。
- 禁止把 Dark Tech 做成“AI 通用模板感”的霓虹素材页。
- 禁止深色背景上再堆太多卡片，导致整页没有呼吸感。
- 禁止把所有内容都做成卡片列表；系统图、流程图、拓扑页应该占更高权重。

## 图表与图形语法

- 图表优先服务于“性能、吞吐、能力差异、系统流动”，而不是普通业务周报。
- 优先使用 **system topology / process pipeline / KPI wall / timeline / architecture hierarchy**。
- 深色图表保持低噪声：网格线弱、标签浅、主系列高亮。
- 如果页面已经有强视觉 SVG，旁边文字必须减量，避免同时争夺注意力。
