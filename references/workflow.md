...
**视觉结构要求（deck 级别，非每页强制）**：
- 整套 PPT 的内容页中，**≥ 60%** 应包含非纯文字视觉组件（SVG 图标、数据卡、流程图、对比表格）
- 允许 40% 的内容页以"文字 + 简单几何前缀"表达信息密集型内容，这在汇报型 PPT 中是合理的，**不要为了凑视觉指标而在已密集的页面上强行叠加图形**
- **禁止在单页中同时出现两种大型图形组件**（如进度环 + 流程图、矩阵 + 金字塔），内容区只有 3.95"，叠加必定溢出
...
- **以下组件仅在内容本身具有该逻辑结构时才使用**：进度环（目标完成率场景）、时间轴（按时间顺序的场景）、金字塔（层级结构场景）；禁止用于"看起来好看"的装饰目的
