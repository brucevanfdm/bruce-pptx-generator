...
数据分析风默认应按“发现 -> 解释 -> 行动”推进：

1. Key Finding：先告诉读者最重要的发现
2. Metric Context：说明口径、时间窗、样本范围
3. Evidence：1 个主图表或主表格
4. Interpretation：3–5 条洞察，解释为什么会这样
5. Implication：这对业务、产品、运营意味着什么
6. Appendix：维度明细、样本拆分、口径补充

不要让读者自己在图里找重点，重点必须由标题、标注和洞察区直接说出来。

## 签名页

这套风格的 signature pages 应优先是：

- **Single-Insight Chart Slide**：1 张主图 + 右侧洞察
- **KPI Dashboard Slide**：4–6 个指标卡，带变化和解释
- **Variance / Before-After Slide**：差异、提升、下降的拆解页
- **Appendix Table Slide**：长表格、多维拆分、样本明细

如果数据分析风没有 appendix，它通常会在主体部分变得拥挤。

## 禁用模式

- 禁止一页放两个互不相关的主图表。
- 禁止图表没有时间范围、样本口径或来源说明。
- 禁止 KPI 只给数字不给变化、基准或业务含义。
- 禁止长明细表继续留在主叙事中；超出安全密度时直接移入 appendix。

## 图表与图形语法

- 图表必须自带阅读辅助：基准线、变化箭头、异常点标注、颜色语义。
- 默认语义：`secondary` = 主系列，`positive` = 正向，`accent` = 风险/负向。
- 标题必须是结论句，洞察区必须解释“为什么”和“怎么办”。
- 长表格、维度拆分、样本列表优先进入 appendix，并使用自动分页。
