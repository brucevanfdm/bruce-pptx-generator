...
4. **视觉结构必须有（deck 级别，非每页强制）**：整套 PPT 的内容页中，**≥ 60% 应包含**非纯文字视觉组件（SVG 图标、数据卡、流程图、对比表格）。允许 40% 的内容页以"文字 + 简单几何前缀"呈现信息密集型内容——这在汇报型 PPT 中是合理且必要的。
5. **图形密度上限（防溢出，每张幻灯片单独执行）**：
   - 每张幻灯片最多使用**一种大型图形组件**（流程图 OR 矩阵 OR 金字塔，三选一，不叠加）
   - 禁止在同一页中同时出现两种以上复杂图形：进度环 + 矩阵 = 溢出风险极高
   - 内容区高度仅 3.95"（y:1.05 → y:5.0），大型图形占用后剩余空间有限，必须估算文字是否放得下再决定是否添加图形
   - **禁止在本风格中生成以下组件**（需要专用算法，不在预设库内）：进度环（progress ring）、漏斗图（funnel）、甘特图（Gantt）、雷达图（radar chart）
